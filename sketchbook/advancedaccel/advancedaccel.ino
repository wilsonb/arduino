...
int val[3]; //scaled accel value for scope readout
int k;
  ...
  if (digitalRead(int1Pin)==1)  // Interrupt pin, should probably attach to interrupt function
  {
    ...
    */
    ...
    for (int i=0; i<3; i++){
      ...
      val[i] = (accelG[i]/3)*255;   //255 is max accel 5g
    }
    // Print out values
    for (int i=0; i<3; i++)
    {
    ...
    for (int i=0; i<3; i++)
    {
      Serial.print(val[i]);  // Print g values
      analogWrite(i,val[i]);
      Serial.print(" ");  // tabs in between axes
...
void initMMA8452(byte fsr, byte dataRate)
{
  ...
  writeRegister(0x21, 0x55);  // 1. enable single/double taps on all axes
  // writeRegister(0x21, 0x55);  // 1. single taps only on all axes
  // writeRegister(0x21, 0x6A);  // 1. double taps only on all axes
  writeRegister(0x23, 0x40);  // 2. x thresh at 2g, multiply the value by 0.0625g/LSB to get the threshold
  writeRegister(0x24, 0x40);  // 2. y thresh at 2g, multiply the value by 0.0625g/LSB to get the threshold
  writeRegister(0x25, 0x40);  // 2. z thresh at .5g, multiply the value by 0.0625g/LSB to get the threshold
